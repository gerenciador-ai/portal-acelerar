...
async function processCompany(name, apiKey, fileName) {
    console.log(`🚀 Iniciando Sincronização Nibo (${name})...`);
    const CATEGORIA_ALVO = "311014001";
    const url = "https://api.nibo.com.br/empresas/v1/schedules?$top=1000";
    const hoje = new Date();

    try {
        const res = await axios.get(url, { headers: { 'apitoken': apiKey, 'accept': 'application/json' } });
        ...
        console.log(`✅ Sincronização ${name} concluída!`);
    } catch (e) {
...
async function run() {
    await processCompany("VMC Tech", "BBC8B184DE0C41F8BF2EA9162263E72D", "inadimplencia_vmctech.json");
    await processCompany("Victec", "A967D3D9A45E4B8890F0437FEDCF6872", "inadimplencia_victec.json");
}
